...
위 예시에서 100갸의 쓰레드가 같은 상품의 재고를 감소시켰을 때 최종 재고는 0을 기대하지만 실제 테스트는 실패하는 것을 확인 할 수 있디.
...
- Exclusive Lock을 걸게되면 다른 트랜잭션에선느 Lock이 해제되기 전에는 데이터를 가져갈 수 없음
- Dead Lock이 걸릴 수 있기 때문에 주의
...
- 먼저 데이털르 읽은 후 update를 수행할 때 현재 내가 읽은 버전이 맞는지 확인하여 업데이트
...
- Transaction이 종료될 때 자동으로 해제되지 않음. 별도의 명령어로 해제/선점 시간이 끝나야 해제 Transaction이 종료될 때 Lock이 자동으로 해제되지않음.
